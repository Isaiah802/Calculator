...
class SPIManager:
    ...
    def __init__(self):
        self.spi = SPI(
            config.Hardware.SPI_BUS, 
            baudrate=config.Hardware.SPI_BAUDRATE_DISPLAY,
            polarity=0, 
            phase=0,
            sck=Pin(config.Hardware.SPI_SCK),
            mosi=Pin(config.Hardware.SPI_MOSI),
            miso=Pin(config.Hardware.SPI_MISO)
        )
    ...
    def switch_to_display(self):
        ...
        if self.current_mode != 'display':
            try:
                ...
                self.current_mode = 'display'
                logger.debug("SPI switched to display mode")
            except Exception as e:
                logger.error(f"Failed to switch SPI to display: {e}")
                raise HardwareError(f"SPI display switch failed: {e}")
    ...
    def switch_to_sd(self):
        ...
        if self.current_mode != 'sd':
            try:
                self.spi.init(
                    baudrate=config.Hardware.SPI_BAUDRATE_SD,
                    polarity=0, phase=0
                )
                self.current_mode = 'sd'
                logger.debug("SPI switched to SD card mode")
            except Exception as e:
                logger.error(f"Failed to switch SPI to SD: {e}")
                raise HardwareError(f"SPI SD switch failed: {e}")
...
class DisplayManager:
    ...
    def __init__(self, spi_manager: SPIManager):
        ...
        self.width = config.Hardware.DISPLAY_WIDTH
        self.height = config.Hardware.DISPLAY_HEIGHT
        
        # Control pins
        self.cs = Pin(config.Hardware.DISPLAY_CS, Pin.OUT)
        self.dc = Pin(config.Hardware.DISPLAY_DC, Pin.OUT)
        self.rst = Pin(config.Hardware.DISPLAY_RST, Pin.OUT)
        ...
        try:
            ...
            logger.warning("Low memory: falling back to reduced display buffer")
            try:
                ...
                logger.error(f"Unable to allocate framebuffer: {e}")
                self.buffer = None
                self.fb = None
                self.reduced_resolution = True
...
class KeypadManager:
    ...
    def __init__(self):
        self.cols = [Pin(c, Pin.IN, Pin.PULL_UP) for c in config.Hardware.KEYPAD_COLS]
        self.rows = [Pin(r, Pin.OUT) for r in config.Hardware.KEYPAD_ROWS]
    ...
    def scan_keys(self) -> set:
        ...
        for row_idx, row_pin in enumerate(self.rows):
            ...
            # Small settling time
            time.sleep_us(config.Hardware.SCAN_SETTLE_US)
    ...
    def get_events(self) -> List[Tuple[Tuple[int, int], str]]:
        ...
        # Throttle scanning
        if time.ticks_diff(now, self.last_scan) < config.Hardware.DEBOUNCE_MS // 2:
            return events
        ...
        for key in list(self.pressed_keys):
            if key not in current_pressed:
                ...
                if duration >= config.Hardware.LONG_PRESS_MS:
...
class PowerManager:
    ...
    def __init__(self):
        self.adc = ADC(Pin(config.Hardware.BATTERY_ADC))
        self.backlight_pwm = PWM(Pin(config.Hardware.BACKLIGHT_PWM))
...
class CalculatorApp:
    ...
    def __init__(self):
        ...
        self.display = DisplayManager(self.spi_manager)
        self.keypad = KeypadManager()
